...
nn_map_Seurat: given the n_cells x n_neighbors Data Frame from Seurat (cell index start from 1), 
generate the nn_map needed for pseudo-bulk generatiion

nn_map_py: given the n_cells x n_neighbors Data Frame from python packages like scglue (cell index start from 0),
generate the nn_map needed for pseudo-bulk generatiion

nn_map_embed: given the n_cells x n_dims array-like data, calculate the nearest neighbors, 
then generate the nn_map

Parameters:
K: default 20, number of neighbors (including self) to generate the nn_map
"""
...
def _nnmap_by_group(nn_map, group_labels):
    """
    Function to limit the neighbors of each cell be within the same label group
    Parameters:
...
def subset(anndat_mod1, anndat_mod2, nn_map, by, groups, seed=1):
    """
    Function to subset the dataset to spcific label groups

    Parameters:
    anndat_mod1: data module 1
    anndat_mod2: data module 2, paired cells should be in the same order and index as anndat_mod1
...
def add_group_sparsity(anndat, group_by=None):
    ...
    '''
    # old version
    dropout_df = pd.DataFrame((anndat.X != 0) * 1.0)
    dropout_df = pd.DataFrame(dropout_df.mean())
    dropout_df.index = anndat.var_names
    dropout_df.columns = ['Frac.all']
    anndat.var = pd.concat([anndat.var, dropout_df], axis=1)

    if group_by is not None:
        dropout_df = pd.DataFrame((anndat.X != 0) * 1.0)
        dropout_df['clus'] = anndat.obs[group_by].to_numpy()
        dropout_df = dropout_df.groupby('clus').mean()
        dropout_df.columns = anndat.var_names
        dropout_df = dropout_df.T
        dropout_df.columns = 'Frac' + '.' + dropout_df.columns
        anndat.var = pd.concat([anndat.var, dropout_df], axis=1)
    '''
...
def PsedoBulk(anndat_mod1, anndat_mod2, nn_map, 
    ...
    anndat_mod1: single cell AnnData object 1 (main)
                 X should be the raw counts

    anndat_mod2: single cell AnnData object 2 to be processed, 
                 cells should be in the order of their aligned pair in object 1
                 X should be the raw counts
    
    nn_map: (n_cells, n_neighbors) numpy array containing nearest neighbors' index of each cell

    N: number of pseudo-bulks
...
def RNA_preprocessing_in_one(anndat, HVGs=None,
    ...
    """
    Funcion to preprocess the input RNA-seq data
    """

    """ Parameters
    anndat: scRNA-seq AnnData object

    HVGs: gene list to be used in following analysis
...
def ATAC_preprocessing_in_one(anndat,
                              sparse_cutoff=0.05):
    """
    Funcion to preprocess the input ATAC-seq data
    """

    """ Parameters
    anndat: scATAC-seq AnnData object

    sparse_cutoff: minimum sparsity to keep features
    """
...
def multiome_data(anndat_mod1, anndat_mod2, peaks_nearby, raw_counts=False):
    ...
    #if raw_counts:
    #    print("AnnData object genertated with library size normalized counts as X.")
    #    genes_X = anndat_mod1.uns['counts'].copy()[:, gene_mask]
    #    peaks_X = anndat_mod2.layers['counts'].copy()[:, peak_mask]
    #else:
